use crate::{
    ...
    adapters::{WeChatStyleAdapter, ZhihuStyleAdapter, PlatformAdapter},
    cli::{Platform, ConfigAction, TemplateAction},
    cli::args::AppConfig,
};
        ...
        match target_platform {
            Platform::WeChat => {
                ...
                if preview {
                    ...
                    save_output(&processed_content, &adapted_html, &target_platform, &output, &config).await?;
                }
            }
            Platform::Zhihu => {
...
pub async fn publish_command(
    ...
    match platform {
        ...
        Platform::All => {
            return Err(crate::error::Error::Other("发布时不能选择'all'平台".to_string()));
        }
    }
...
pub async fn template_command(action: TemplateAction) -> Result<()> {
    match action {
        ...
        TemplateAction::Apply { name, input, output: _ } => {
...
async fn save_output(
    ...
    let output_dir = output_override.as_ref()
