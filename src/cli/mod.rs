...
pub async fn run() -> Result<()> {
    ...
    match cli.command {
        Commands::Process { input, output, platform, preview } => {
            commands::process_command(input, output, platform, preview).await
        }
        Commands::Watch { directory, output, pattern } => {
            commands::watch_command(directory, output, pattern).await
        }
        Commands::Publish { content, platform, draft } => {
            commands::publish_command(content, platform, draft).await
        }
        Commands::Serve { port, host, static_dir } => {
            commands::serve_command(port, host, static_dir).await
        }
        Commands::Config { action } => {
            commands::config_command(action).await
        }
        Commands::Template { action } => {
            commands::template_command(action).await
        }
    }
}
